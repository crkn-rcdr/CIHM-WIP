    ...
    if ( $report ne '' ) {
        ...
        if ( index( $email, "@" ) == -1 ) {
            ...
            my $message = Email::MIME->create(
                header_str => [
                    From =>
'"Canadiana\'s outdated METSproc Reporter" <noreply@canadiana.ca>',
                    To      => $email,
                    Subject => $subject,
                ],
                attributes => {
                ...
                body_str =>
"This is a horrible outdated tool.\n\nYou really want to use https://access.canadiana.ca/dmd/new instead!\n\n\n"
                  . $report,
            );
            sendmail($message);
        }
            ...
            foreach my $pathname (@paths) {
                ...
                my $thisoutput = capture_merged {
                    ...
                    if ( defined $config->{metsproc} ) {
                        my $metsproc =
                          find_my_metsproc( $pathname, $config->{metsproc} );
                        ...
                        switch ($mycommand) {
                            case "ignore" { }
                            case
                              /^(csv2issue|csv2dc|dbtext2lac|dbtext2news|marc)$/
                            {
                                @command = (
                                    "mets",    # TODO: in config file?
                                    $mycommand
                                );
                                if ( $metsproc->{extraparam} ) {
                                    if (
                                        ref $metsproc->{extraparam} eq 'ARRAY' )
                                    {
                                        push @command,
                                          @{ $metsproc->{extraparam} };
                                    }
                                ...
                                print STDERR
"Command=$mycommand is not understood. configid=$configid, pathname=$pathname\n";
                                $error = 1;
                            }
                        }
                        ...
                        print STDERR
                          "No Metadata Processing type defined for $configid\n";
                        $error = 1;
                    }
                    $self->rename_mdfile( $pathname,
                        ( $error ? $rejectpath : $trashpath ) );
                };
...
sub rename_mdfile {
    ...
    my ( $vol, $dirs, $filename ) = File::Spec->splitpath($source);
    ...
    rename $source,
      $destname || die "Could not rename $source to $destname: $!\n";
