...
sub imageconv {
    ...
    $self->log->info("Imageconv: conf=".$self->configpath." skip=".$self->skip. " limit=".$self->limit. " maxprocs=" . $self->maxprocs . " timelimit=".$self->{timelimit});
...
sub getNextAIP {
    ...
    my $res = $self->wipmeta->get("/".$self->wipmeta->{database}."/_design/tdr/_view/imageconvq?reduce=false&limit=".$self->limit.$skipparam,{}, {deserializer => 'application/json'});
    if ($res->code == 200) {
        ...
        warn "_view/imageconvq GET return code: ".$res->code."\n"; 
    }
    return;
}
