...
option 'report' => (
  ...
  isa => 'Str',
  default => "",
  documentation => q[Comma separate list of email addresses that should receive report. If no @ then standard output],
);
...
sub run {
  ...
  my $stages = $self->WIP->stages || die "Missing <stages> section of config file\n";
...
sub walk_identifiers {
    ...
    $self->log->info("Scan for identifiers for stage $name at $path/$configid\n");

    if (opendir (my $dh, "$path/$configid")) {
        while(readdir $dh) {
            next if $_ eq "." || $_ eq "..";
            next if $_ eq "dirlist.txt" || $_ eq "imagelist.txt" || $_ eq "ReelImageData.csv";
            my $identifier=$_;

            my $idpath=$path."/".$configid."/".$identifier;
            if (-d $idpath) {
                if (index($identifier,'.') != -1) {
                    warn "Identifer '$identifier' at $name/$configid shouldn't have a period\n";
                }
                ...
                if ($self->WIP->objid_valid($objid)) {
                    ...
                    if (exists $self->onfs->{$name}->{$configid}->{$identifier}) {
                        $skipdb = $self->onfs->{$name}->{$configid}->{$identifier} eq $aip;
                    }
                    if ($name ne 'Trashcan') {
                        my $nclassify=$self->classify_obj_dir($configid,"$path/$configid/$identifier");
                        ...
                        if(exists $self->aipfs->{$aip}) {
                            ...
                            if ($skipdb) {
                                $skipdb=hasSameClassify($nclassify,$oclassify);
                            }
                        } else {
                            $skipdb=0;
                        }
                            ...
                            foreach my $stage (@stages) {
                                if ($stage ne $name) {
                                    if (-d $self->WIP->stages->{$stage}."/".$configid."/".$identifier) {
                                        warn("Duplicate: $configid/$identifier exists in both $name and $stage\n");
                                    }
                                }
                            }
                        }
                ...
                warn "Identifier $identifier not directory at: $name/$configid/$identifier\n";
            }
        }
        closedir $dh;
    } else {
...
sub walk_dbfilesystem {
    ...
    foreach my $fsid (@{$filesystem}) {
        my $docid = $fsid->{id};
        my ($objid,$stage,$identifier) = @{$fsid->{key}};
        $stage='Trashcan' if (!$stage); # Blank/undefined stage shouldn't be in database, so this will fix old entries.
        if (-d $stages->{$stage}) {
            if (! -d $stages->{$stage}."/".$objid."/".$identifier) {
                if ($stage ne 'Trashcan') {
                    warn $stage."/".$objid."/".$identifier." no longer exists\n";
                }
                my $retdata = $self->WIP->wipmeta->update_filesystem(
                    $docid,
                    {
...
sub gen_report {
    ...
    $res = $self->WIP->wipmeta->get(
        "/".$self->WIP->wipmeta->database."/_design/tdr/_view/filesystem?group_level=2", 
        {}, 
    ...
    #
    $res = $self->WIP->wipmeta->get(
        "/".$self->WIP->wipmeta->database."/_design/tdr/_view/manifestdate?group_level=3&limit=10&descending=true",
        {},
        {deserializer => 'application/json'});
    if (!$res->data && $res->code != 200) {
        die "_view/filesystem GET return code: " . $res->code ." (".$res->response->content.")\n";
    ...
    if ($self->WIP->tdrepo) {
        $res = $self->WIP->tdrepo->get(
            "/".$self->WIP->tdrepo->database."/_design/tdr/_view/repofilesize?group_level=1",
            {},
            {deserializer => 'application/json'});
        if (!$res->data && $res->code != 200) {
            die "_view/repofilesize GET return code: " . $res->code ." (".$res->response->content.")\n";
        }
        ...
        if (scalar(@{$res->data->{rows}})) {
            ...
            foreach my $row (@{$res->data->{rows}}) {
                ...
                $report .= " * '$repository' has $count AIPs using $sizehr ($size bytes) of storage\n";
            }
        }
    }
