...
sub update_filesystem {
  ...
  # This encoding makes $updatedoc variables available as form data
  $self->type("application/x-www-form-urlencoded");
  $res = $self->post("/".$self->database."/_design/tdr/_update/filesystem/".$uid, $updatedoc, {deserializer => 'application/json'});
...
sub get_configdocs {
  ...
  $res = $self->get("/".$self->database."/_design/tdr/_view/configs?reduce=false&include_docs=true", {}, {deserializer => 'application/json'});
...
sub get_filesystem {
  ...
  $res = $self->get("/".$self->database."/_design/tdr/_view/filesystem?reduce=false&include_docs=".($docs?"true":"false"), {}, {deserializer => 'application/json'});
...
sub get_attachment {
    ...
    $self->type("application/json");
    my $res = $self->get("/".$self->{database}."/$uid/$attachment",{}, {deserializer => 'application/json'});
    if ($res->code == 200) {
